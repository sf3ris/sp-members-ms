...
class MembersPDF():
    ...
    def generate_pdf(self, members: List[Member], columns: List[str]) -> bytes:
        ...
        if('name' in columns):
            pdf.cell(col_width, th, 'Nome', border=1, fill=True)
        if('last_name' in columns):
            pdf.cell(col_width, th, 'Cognome', border=1,fill=True)
        if('birth_date' in columns):
            pdf.cell(birth_date_width, th, 'Data di nascita', border=1, fill=True)
        if('birth_place' in columns):
            pdf.cell(col_width, th, 'Luogo', border=1, fill=True)
        ...
        if('membership' in columns):
            pdf.cell(col_width, th, 'Scadenza', border=1, fill=True)
        if('status' in columns):
            pdf.cell(col_width, th, 'Stato', border=1, fill=True)
        pdf.ln(th)
        ...
        for member in members:
            ...
            if('status' in columns):
                pdf.cell(
                    col_width,
                    th,
                    'Attivo' if member.is_enabled() else 'Non Attivo',
                    border=1,
                    fill=True
                )
            pdf.ln(th)
