...
def update_member(member_id : str) -> flask.Response:
    ...
    if birth_date   is not None : member.birth_date = birth_date
    if birth_place  is not None : member.birth_place = birth_place
    if fiscal_code  is not None : member.fiscal_code = fiscal_code
    if address      is not None : member.address = address
    if zip_code     is not None : member.zip_code = zip_code
    if city         is not None : member.city = city
    if province     is not None : member.province = province
    if gender       is not None : member.gender = gender
    if phone        is not None : member.phone = phone
